...
// ── MAIN TAB SWITCH ──
window.switchMainTab = function(tab, event) {
...
function renderDashboard(filtered) {
  ...
  deptChart = new Chart(ctx1, {
    type: "bar",
    data: { labels: Object.keys(deptCounts), datasets: [{ label: "Visitors", data: Object.values(deptCounts), backgroundColor: "#3b82f6" }] },
    options: { responsive: true, maintainAspectRatio: false }
  });

  // Program chart
  const progCounts = {};
  filtered.forEach(v => { if(v.course && v.course !== "N/A" && v.course !== "N/A (Employee)") progCounts[v.course] = (progCounts[v.course]||0)+1; });
  ...
  programChart = new Chart(ctx2, {
    type: "bar",
    data: { labels: Object.keys(progCounts), datasets: [{ label: "Visitors", data: Object.values(progCounts), backgroundColor: "#8b5cf6" }] },
    options: { responsive: true, maintainAspectRatio: false }
  });
...
window.renderAnalytics = function(data) {
  ...
  trendChart = new Chart(ctx, {
    type: "line",
    data: { labels: trendLabels, datasets: [{ label: "Visitors", data: trendValues, borderColor: "#3b82f6", backgroundColor: "rgba(59,130,246,0.1)", tension: 0.3, fill: true, pointRadius: 3 }] },
    options: { responsive: true, maintainAspectRatio: false }
  });
  ...
  anDeptChart = new Chart(ctx2, {
    type: "bar",
    data: { labels: deptSorted.map(d=>d[0]), datasets: [{ label: "Visitors", data: deptSorted.map(d=>d[1]), backgroundColor: "#3b82f6" }] },
  ...
  purposePieChart = new Chart(ctx3, {
    ...
    data: {
      ...
      datasets: [{ data: Object.values(purposeCounts), backgroundColor: COLORS }]
    },
    options: { responsive: true, maintainAspectRatio: false }
  });
  ...
  breakdown.innerHTML = Object.entries(purposeCounts).sort((a,b)=>b[1]-a[1]).map(([name,val],i) => `
    <div class="purpose-item">
      <div class="purpose-item-header">
        ...
        <span style="font-size:0.875rem;font-weight:600;">${val}</span>
      </div>
  ...
  const peakData = Array.from({length:24},(_,i)=>({ hour: i<12?`${i||12}AM`:`${i===12?12:i-12}PM`, visits: hours[i]||0 }));
  const ctx4 = document.getElementById("peakHoursChart").getContext("2d");
  if (peakChart) peakChart.destroy();
  peakChart = new Chart(ctx4, {
    type:"bar", data:{labels:peakData.map(d=>d.hour),datasets:[{label:"Visits",data:peakData.map(d=>d.visits),backgroundColor:"#8b5cf6"}]},
    options:{responsive:true,maintainAspectRatio:false}
  });
  ...
  const dowData = days2.map((n,i)=>({name:n,visits:dowCounts[i]||0}));
  const ctx5 = document.getElementById("dayOfWeekChart").getContext("2d");
  if (dowChart) dowChart.destroy();
  dowChart = new Chart(ctx5, {
    type:"bar", data:{labels:dowData.map(d=>d.name),datasets:[{label:"Visits",data:dowData.map(d=>d.visits),backgroundColor:"#10b981"}]},
    options:{responsive:true,maintainAspectRatio:false}
  });
  ...
  document.getElementById("insightsGrid").innerHTML = `
    <div class="insight-card insight-blue">
      ...
      <p class="insight-sub">${busiest?.visits||0} average visits</p>
    </div>
    <div class="insight-card insight-purple">
      <h4>Peak Hour</h4>
      <p class="insight-value">${peakHour?.hour||"—"}</p>
      <p class="insight-sub">Most popular time to visit</p>
    </div>
    <div class="insight-card insight-green">
      <h4>Top Purpose</h4>
      <p class="insight-value">${topPurpose?.[0]||"—"}</p>
      <p class="insight-sub">Most common visit reason</p>
    </div>
  ...
  if (isMobile) {
    ...
    body.innerHTML = paginated.map(log => `
      <tr>
        ...
        <td style="color:#6b7280">${new Date(log.timestamp).toLocaleString([],{month:"short",day:"numeric",year:"numeric",hour:"2-digit",minute:"2-digit"})}</td>
      </tr>
    `).join("");
  }
...
window.exportCSV = function() {
  const csv = [
    ["Name","Email","Department","Course","Purpose","Type","Date & Time"].join(","),
    ...filteredVisits.map(v => [
      v.name, v.email, v.department, v.course, v.purpose, v.visitorType,
